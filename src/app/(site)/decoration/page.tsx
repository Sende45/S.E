...
import { Sparkles, Armchair, Lightbulb, ArrowRight } from "lucide-react";

const prestations = [
  { icon: Sparkles, titre: "Décor de cérémonie", desc: "Arches, allées, fonds de scène et compositions florales sur mesure." },
  { icon: Armchair, titre: "Dressage & mobilier", desc: "Nappage, vaisselle, housses de chaises et centres de table coordonnés." },
  { icon: Lightbulb, titre: "Lumière & ambiance", desc: "Guirlandes, projecteurs et jeux de lumière pour sublimer l'espace." },
];

const galerie = [
  { titre: "Arches florales", from: "#7a4a6b", to: "#34122c", big: false },
  { titre: "Dressage de tables", from: "#5b2a4e", to: "#2a0f24", big: true },
  { titre: "Mise en lumière", from: "#8a5a2a", to: "#4a1d3f", big: false },
];
  ...
  return (
    <>
      <section className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-6xl px-6 pt-20 pb-12">
          ...
          <h1 className="max-w-[16ch] font-display text-4xl font-semibold leading-[1.03] md:text-6xl">
            Une <em className="italic text-[var(--gold)]">décoration</em> élégante et sur-mesure
          </h1>
          <p className="mt-6 max-w-[56ch] text-lg leading-relaxed text-[var(--muted)]">
            Nous transformons vos lieux en décors d&apos;exception : arches
            florales, dressage de tables, drapés, mise en lumière et ambiances
            personnalisées selon votre thème.
          </p>
        </div>
      </section>
      ...
      <section className="bg-[var(--bg-2)] py-14 text-center">
        <span className="inline-block h-1.5 w-1.5 rotate-45 bg-[var(--gold)]" />
        <h2 className="mt-4 font-display text-3xl font-semibold">Une ambiance à imaginer ?</h2>
        <div className="mt-6">
          <Link href="/devis?pole=decoration" className="inline-flex items-center gap-2 rounded-lg bg-[linear-gradient(135deg,var(--gold),var(--gold-deep))] px-6 py-3.5 text-sm font-bold text-[#3A1631] shadow-[0_6px_18px_rgba(201,162,39,0.25)] transition-transform hover:-translate-y-0.5">
            Demander un devis décoration <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </section>
    </>
