...
import { CategorieRealisation } from "@prisma/client";
import {
  Sparkles,
  Armchair,
  Lightbulb,
...
const prestations = [
  {
    icon: Sparkles,
    titre: "Décor de cérémonie",
    desc: "Arches, allées, fonds de scène et compositions florales sur mesure.",
  },
  {
    icon: Armchair,
    titre: "Dressage & mobilier",
    desc: "Nappage, vaisselle, housses de chaises et centres de table coordonnés.",
  },
  {
    icon: Lightbulb,
    titre: "Lumière & ambiance",
    desc: "Guirlandes, projecteurs et jeux de lumière pour sublimer l'espace.",
  },
];
...
export default async function DecorationPage() {
  const galerie = await prisma.realisation.findMany({
    where: {
      categorie: CategorieRealisation.DECORATION,
    },
    orderBy: {
      createdAt: "desc",
    },
    ...
    <>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-6xl px-6 pt-20 pb-12">
          ...
          <h1 className="max-w-[16ch] font-display text-4xl font-semibold leading-[1.03] md:text-6xl">
            Une <em className="italic text-[var(--gold)]">décoration</em>{" "}
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            {galerie.map((g, index) => (
              <div
                ...
                style={{
                  aspectRatio: index === 1 ? "auto" : "3 / 4",
                  minHeight: index === 1 ? "260px" : undefined,
                }}
              >
                <Image
                  src={g.imageUrl}
                  alt={g.titre ?? "Décoration"}
                  fill
      ...
      <section className="bg-[var(--bg-2)] py-14 text-center">
        ...
        <h2 className="mt-4 font-display text-3xl font-semibold">
          Une ambiance à imaginer ?
        </h2>

        <div className="mt-6">
          <Link
            href="/devis?pole=decoration"
