...
// Numéro WhatsApp de S.E (format international sans +, ni espaces)
...
export default function ProductGrid({
  ...
  const liste =
    actif === "Tout" ? produits : produits.filter((p) => p.cat === actif);
    ...
    <>
      ...
      <div className="grid grid-cols-2 gap-5 md:grid-cols-4">
        {liste.map((p) => (
          ...
          >
            <div
              className="relative grid aspect-square place-items-center text-5xl"
            ...
            <div className="p-4">
              ...
              <h3 className="mt-1 font-display text-lg font-semibold">{p.nom}</h3>
              <div className="mt-2 text-sm font-bold">
                {p.prix.toLocaleString("fr-FR")}{" "}
                <span className="text-xs font-medium text-[var(--faint)]">FCFA</span>
              </div>
              <button
